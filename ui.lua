...
local function ShowKeyScreen(onSuccess)
    ...
    contCorner.CornerRadius = UDim.new(0, 20)
    ...
    submitBtn.BackgroundColor3 = Color3.fromRGB(50, 255, 90)
    ...
    submitBtn.MouseButton1Click:Connect(function()
        ...
        local validKeys = VerifyKey()
        
        local isCorrect = false
        if type(validKeys) == "table" then
            for _, v in pairs(validKeys) do
                if v == inputKey then isCorrect = true break end
            end
        elseif type(validKeys) == "string" then
            if validKeys == inputKey then isCorrect = true end
        end

        if isCorrect then
            statusText.Text = "Correct Key!"
            statusText.TextColor3 = Color3.fromRGB(50, 255, 90)
            TweenService:Create(container, TweenInfo.new(0.5), {Size = UDim2.new(0,0,0,0), Position = UDim2.new(0.5, 0, 0.5, 0)}):Play()
            blur:Destroy()
            task.wait(0.5)
            keyGui:Destroy()
            onSuccess()
            ...
            local shake = TweenService:Create(inputBox, TweenInfo.new(0.1, Enum.EasingStyle.Quad), {Position = UDim2.new(0, 25, 0, 100)})
            shake:Play()
            shake.Completed:Wait()
            TweenService:Create(inputBox, TweenInfo.new(0.1), {Position = UDim2.new(0, 15, 0, 100)}):Play()
        end
    end)
end
